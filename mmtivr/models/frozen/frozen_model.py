...
#
# Adapted from https://github.com/m-bain/frozen-in-time
...
from mmitivr.core.checkpoint import state_dict_data_parallel_fix
...
class FrozenInTime(nn.Module):
    ...
    def forward(self, frames, captions):
        self.bz = frames.shape[0]
        text_embeddings = self.compute_text(captions)
        video_embeddings = self.compute_video(frames)
    ...
    def compute_text(self, text_data):
        ...
        text_embeddings = einops.rearrange(text_embeddings, '(b n) d -> b n d', b=self.bz)
